...
export let useGameStore = defineStore("game", {
  ...
  state: () => ({
    ...
    quasar: null,
    config: {
      projectTiming: 10, // a 1:1 seniority:complexity timing for a project
      projectSeekingTiming: 5, // timing for a senior salesman to seek a new project
      projectValueFactor: 1, // based on dev salary value
      maxDevSalary: 40, // maximum developer salary (per second)
      maxSaleSalary: 55, // maximum salesman salary (per second)
      maxShopDevs: 3, // 
      maxShopSales: 3, //
    },
    shop: {
  ...
  actions: {
    ...
    completeProject(project) {
      ...
      this.quasar.notify({
        ...
        type: 'positive',
        html: true,
        message: `il progetto <strong>${project.name}</strong> è stato completato!`
      })
      ...
      this.quasar.notify({
        ...
        type: 'info',
        html: true,
        message: `Hai guadagnato <strong>€ ${project.amount}</strong>`
      })
    ...
    async generateRandomPerson(type='developer', level=0) {
      ...
      if(type == 'developer') {
        ...
        return {
          id: random.results[0].login.uuid,
          name: `${random.results[0].name.first} ${random.results[0].name.last}`,
          experience: getRandomArbitrary(1,3),
  ...
  getters: {    
    inprogressProjects() {
      return this.projects.filter((d) => d.status == 'in progress');
    },

    incompleteProjects() {
      return this.projects.filter((d) => d.status != 'completed');
    },
